...
### Claude 賛成派（Agentツール, subagent_type: "debater"）
- promptに討論テーマ、立場「賛成（この立場を支持する論拠を展開せよ）」、出力先ファイルパス `{WS}/1_claims/pro_claude.md` を渡す

### Claude 反対派（Agentツール, subagent_type: "debater"）
...
- `gemini -p "$(cat {WS}/1_claims/.prompt_pro_gemini.txt)" > {WS}/1_claims/pro_gemini.md` を実行

### Gemini 反対派（Bashツール）
- 同上、反対用に置換
- 一時ファイル: `{WS}/1_claims/.prompt_con_gemini.txt`
- 出力先: `{WS}/1_claims/con_gemini.md`

### Codex 賛成派（Bashツール）
- debater.mdテンプレートを賛成用に置換し、一時ファイル `{WS}/1_claims/.prompt_pro_codex.txt` に書き出す
- `codex exec "$(cat {WS}/1_claims/.prompt_pro_codex.txt)" 2>/dev/null > {WS}/1_claims/pro_codex.md` を実行

### Codex 反対派（Bashツール）
- 同上、反対用に置換
- 一時ファイル: `{WS}/1_claims/.prompt_con_codex.txt`
- 出力先: `{WS}/1_claims/con_codex.md`

### エラーハンドリング
GeminiまたはCodexのCLI実行が失敗した場合、該当ファイルに `（取得失敗: CLIエラー）` と書き込んで続行する。
...
以下の2タスクを **同一レスポンス内で並列に** Agentツール（subagent_type: "summarizer"）で発行する。
...
Agentツール（subagent_type: "judge"）で以下の1タスクを発行する。
